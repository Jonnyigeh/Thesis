...
if olap:
    ...
    im = ax.imshow(S, cmap=custom_cmap, vmin=0, vmax=1)
    # ax.grid(False)
    ...
    ax.set_xticklabels([f"$\psi_{{{j}}}^{{\\mathrm{{true}}}}$" for j in range(n_levels)])
    ax.set_yticklabels([f"$\psi_{{{i}}}^{{\\mathrm{{DVR}}}}$" for i in range(n_levels)])
...
if spectrum:
    ...
    # Plot the deviance
    ax[1].plot(np.arange(n_levels), dev, label='Deviance')
    ax[1].set_xlabel('Energy level')
    ax[1].set_ylabel(r'Deviance $|E_{DVR} - E_{exact}|$')
    ...
    ax[0].set_xlim(xpos - width -0.2 , xpos + width + 0.2)
    ax[0].axes.xaxis.set_ticklabels([])
    ax[0].set_ylabel('Energy')
    ax[0].set_title('Energy spectrum')
    ax[0].legend(bbox_to_anchor=(1.05, 1), )

    fig.suptitle('DVR vs. True energy spectrum', fontsize=16)
