...
\end{align*}
For a time-dependant Hamiltonian, the Crank-Nicolson method can be viewed as a midpoint evaluation of the Hamiltonian, where the midpoint of the Hamiltonian $H = 1/2(H(t+\Delta t) + H(t))$ is used to compute the time-evolution operator to enforce unitarity, and is especially important for rapidly varying Hamiltonians. 
...
def crank_nicolson(H_t, psi, t, dt):
    ...
    H_current = H_t(t)
    H_next = H_t(t + dt)
    H_mid = (H_current + H_next) / 2  # Midpoint evaluation for unitarity
...
\end{lstlisting}
\textcolor{red}{TODO: Comment on hwo to regain unitarity with the operator given that $H(t + \Delta t) \neq H(t)$. Use midpoint evaluation? I.e $H_mid = \frac{H(t) + H(t + \Delta t)}{2}$ solves the issue, and would make the operator unitary. But is it wrong? Should discuss with Oskar and find some sources}
